...
export function exampleZhTranslation(date: string): { markdown: string } {
  const markdown = `---
...
- \`ev-1\` [primary] [Supplier A quarterly earnings call transcript](https://example.com/supplier-a-earnings-call) (${date})
- \`ev-2\` [primary] [Supplier A investor presentation](https://example.com/supplier-a-ir-deck) (${date})
- \`ev-3\` [secondary] [Reuters: packaging bottleneck in HBM supply](https://example.com/reuters-hbm-packaging) (${date})
`;
  return { markdown };
}
...
export function fixturesForDate(date: string): Record<string, unknown> {
  return {
    ...
    translate: exampleZhTranslation(date),
